...
def load_dupack_data(df, train_ratio, valid_ratio):
    # df = data_file
    # feature_column = df.columns[:14]
    # raw_feature = df[feature_column]
    # scaler = StandardScaler()
...
def train_dupack(train_dataset, valid_dataset, input_dim, output_dim, num_epochs, batch_size, learning_rate):
    """
...
def plot_dupack(train_losses, valid_losses, train_accuracies, valid_accuracies, train_f1, valid_f1, save_path):
    """
    ...
    axes[2].legend()
    plt.suptitle('DupACK Performance of CNN-LSTM-ATT Model', fontsize=18)

    plt.tight_layout()
    plt.savefig(save_path)


def test_dupack(model, test_dataset, batch_size, device, save_path):
    """
